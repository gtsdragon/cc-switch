...
fn collect_content_value(texts: &mut Vec<TextEntry>, path: Vec<String>, content: &serde_json::Value) {
...
mod tests {
    ...
    fn extracts_claude_messages_and_system() {
        ...
        assert_eq!(values, vec!["system prompt", "hello", "world", "tool output"]);
    }
    ...
    fn extracts_responses_api_input() {
        ...
        let texts = extract_texts_from_body(&body);
        let values: Vec<&str> = texts.iter().map(|(_, t)| t.as_str()).collect();
        assert_eq!(values, vec!["be helpful", "my email is a@b.com", "secret data"]);
    }
