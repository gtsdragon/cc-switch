...
- **Privacy Filter No Longer Turns Itself Off After Testing**: The privacy filter toggle only wrote to local component state and required an explicit save, while the panel unmounts and remounts whenever you leave and re-enter the Privacy tab (or reopen Settings). That remount re-read the config from disk and silently rolled an on but unsaved toggle back to off. The switch now persists immediately and a local-edit guard keeps a fresh backend read from clobbering what you just changed. Separately, `set_privacy_filter_config` wrote the enabled flag to the database *before* starting the service, so a failed start (port in use, missing bundled binary) left the DB claiming the filter was on while the service was not — the UI then read a config that did not reflect reality. The command now brings the service up or down first and only persists the flag once that succeeds.
